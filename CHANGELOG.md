...
- Dockerfile uses `ghcr.io/home-assistant/base:3.24`. Supervisor 2026.04 no longer supplies `BUILD_FROM`, so `FROM $BUILD_FROM` failed to install.
...
- Removed unused `expose_sensors_OID_base`. Entity OIDs are `NET-SNMP-EXTEND-MIB::nsExtendOutput1Line."<entity_id>"`. Delete the old key from saved YAML if start fails after upgrade.
